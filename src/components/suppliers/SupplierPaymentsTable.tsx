    ...
    if (payments.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center p-12 text-center">
                ...
                <p className="text-slate-500 max-w-xs mx-auto text-sm">There are no payment installments recorded for this period.</p>
            </div>
        );
    }
                ...
                <TableBody>
                    {payments.map((payment) => (
                        <TableRow key={payment.id} className="hover:bg-slate-50/50 transition-colors">
                            ...
                            <TableCell className="font-mono text-[11px] text-slate-500">{payment.id}</TableCell>
                            <TableCell>
                                <Badge variant="outline" className="capitalize text-[10px] font-bold py-0.5 border-slate-200 bg-slate-50">
